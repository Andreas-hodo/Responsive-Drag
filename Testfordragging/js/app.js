   ...
   Dragstart(dragId,callback){
    ...
    this.drag_btn.addEventListener("pointerdown",(e)=>{
        if(this.drag_btn.draggable){
            let zoomLevel = Math.round(window.devicePixelRatio * 100) / 100;
    ...
    if(this.drag_btn.classList.contains(this.dragId) && this.drag_btn.draggable){
         e.preventDefault()
    this.my_id.style.transform = `translate(${this.StartX}px,${this.StartY}px)`
     let x = (e.screenX- this.StartScreenX + this.buttonX) / this.zoomLevel
    let  y =  (e.screenY- this.StartScreenY + this.buttonY) / this.zoomLevel
    ...
    if(this.drag_btn.classList.contains(this.dragId) && this.drag_btn.draggable){
        e.preventDefault()  
    this.my_id.style.transform = `translate(${this.StartX}px,${this.StartY}px)`
    let  x = (e.screenX - this.StartScreenX + this.StartX) / this.zoomLevel
    let  y =  (e.screenY - this.StartScreenY + this.StartY) / this.zoomLevel
    this.my_id.style.transform = `translate(${x}px,${y}px)`
    callback(this.my_id)
    }
    })
    ...
    if(this.drag_btn.classList.contains(this.dragId) && this.drag_btn.draggable && e.targetTouches[0].identifier == 0){
    if(e.cancelable){
       e.preventDefault() 
    }   
    let  x = (e.targetTouches[0].screenX - this.StartScreenX + this.StartX) / this.zoomLevel
    let  y = (e.targetTouches[0].screenY - this.StartScreenY + this.StartY) / this.zoomLevel
    this.my_id.style.transform = `translate(${x}px,${y}px)`
    callback(this.my_id) 
    }
    })  
