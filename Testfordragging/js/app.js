   ...
   Dragstart(dragId,callback){
    ...
    this.drag_btn.addEventListener("pointerdown",(e)=>{
        if(this.drag_btn.classList.contains(this.draggablelabel) && this.drag_btn.draggable && this.enabled.start){
            ...
            let left = this.my_id.getBoundingClientRect().left 
            let top = this.my_id.getBoundingClientRect().top 
            let btnLeft = this.drag_btn.getBoundingClientRect().left
            let btntop = this.drag_btn.getBoundingClientRect().top
            let distanceX = left + (this.drag_btn.clientWidth * (ResEvent.getNums("scale")) / 2) + (btnLeft - left) - this.StartX
            let distanceY = top + (this.drag_btn.clientHeight * (ResEvent.getNums("scale")) / 2) + (btntop - top) - this.StartY
            let coordinates = ResEvent.getNums("translate")
            let finalplaceX = coordinates[0] - distanceX 
            let finalplaceY = coordinates[1] - distanceY     
            this.buttonX = finalplaceX
            this.buttonY = finalplaceY 
            this.my_id.style.transform = `translate(${finalplaceX}px,${finalplaceY}px)  scale(${ResEvent.getNums("scale")}) rotate(${ResEvent.getNums("rotate")}deg)`
            callback(this.my_id)
        }
    },{passive:false})
    }
    ...
    if(this.drag_btn.classList.contains(this.dragId) && this.drag_btn.classList.contains(this.draggablelabel) && this.drag_btn.draggable && this.enabled.move){
        ...
        this.my_id.style.transform = `translate(${this.newX}px,${this.newY}px) scale(${ResEvent.getNums("scale")}) rotate(${ResEvent.getNums("rotate")}deg)`
        callback(this.my_id)
    }   
    })
    ...
    this.drag_btn.addEventListener("pointerup",(e)=>{
        ...
        if(belowId){
         if(!belowId.classList.contains(dropId) || this.parent.id == belowId.id){  
            this.drag_btn.classList.remove(this.dragId)
               this.my_id.style.transform = this.startPos + `scale(${ResEvent.getNums("scale")})` + `rotate(${ResEvent.getNums("rotate")}deg)`
               this.dropParent = null

        }else if(belowId.classList.contains(dropId)){
            this.dropParent = belowId
            this.my_id.style.transform = ResEvent.getTransform("translate") + `scale(${ResEvent.getNums("scale")})` + `rotate(${ResEvent.getNums("rotate")}deg)`
        }
        }  
