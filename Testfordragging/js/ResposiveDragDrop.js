...
class DragDrop{
   ...
   DragEnd(callback){
      document.addEventListener("pointerup",(e)=>{
        ...
        let isElem =  e.target == this.drag_btn || e.target.parentNode == this.drag_btn ? true : false
        if(this.drag_btn.draggable && this.enabled.end && ((isElem || outOfBounds) && this.enabled.end)){
          callback()
        }
      })  
    }
